...
@testset "Differentially heated cavity Ra=1e3" begin
    # Problem parameters ---------------------------------------------------------
    Ra = 1.0e3
    Pr = 0.71
    ΔT = 1.0
    ...
    @info "Differentially heated cavity benchmark" grid="$(nx)x$(ny)" Ra=Ra Pr=Pr
    ...
    @test abs(Nu_mean - Nu_ref) / Nu_ref ≤ 0.05
    @test abs(u_mid_dimless - u_ref) / u_ref ≤ 0.1
    @test abs(v_mid_dimless - v_ref) / v_ref ≤ 0.1
