...
"""
Diphasic 2D heat diffusion benchmark with a Robin jump interface condition.
...
Heat2Ph2DRobinParams(; lx=8.0, ly=8.0, x0=0.0, y0=0.0, center=(4.0, 4.0),
                     radius=1.0, Tend=0.1, Dg=1.0, Dl=1.0, He=1.0,
                     cg0=1.0, cl0=0.0, alpha=1.0, beta=1.0, g_gamma=0.0) =
    Heat2Ph2DRobinParams(lx, ly, x0, y0, center, radius, Tend, Dg, Dl, He, cg0, cl0, alpha, beta, g_gamma)
...
psi_val(u, params::Heat2Ph2DRobinParams) = begin
    D = sqrt(params.Dg / params.Dl)
    R = params.radius
...
function heat2d_phase2_solution(params::Heat2Ph2DRobinParams)
    ...
    return (x, y) -> begin
        r = hypot(x - params.center[1], y - params.center[2])
        r < params.radius && return 0.0
        val, _ = quadgk(u -> cl_integrand(u, x, y, params), 0, Umax; atol=1e-8, rtol=1e-8)
        prefactor * val
    end
end
...
function run_heat_2ph_2d(
    nx_list::Vector{Int};
    params::Heat2Ph2DRobinParams=Heat2Ph2DRobinParams(),
    norm::Real=2,
    relative::Bool=false
)
    h_vals = Float64[]
    ...
    for nx in nx_list
        ...
        # Interface law: alpha * [[T]] + beta * (grad T · n) = g_gamma
        bc_i = RobinJump(params.alpha, params.beta, params.g_gamma)
        bc_f = FluxJump(params.Dg, params.Dl, 0.0)
        ic = InterfaceConditions(bc_i, bc_f)
        ...
        solver = DiffusionUnsteadyDiph(phase1, phase2, bc_b, bc_i, bc_f, Δt, u0, "BE")
        solve_DiffusionUnsteadyDiph!(solver, phase1, phase2, Δt, params.Tend, bc_b, bc_i, bc_f, "BE"; method=Base.:\)
...
function main(; csv_path=nothing, nx_list=nothing, params::Heat2Ph2DRobinParams=Heat2Ph2DRobinParams())
    nx_vals = isnothing(nx_list) ? [8, 16, 32, 64, 128] : nx_list
    data = run_heat_2ph_2d(nx_vals; params=params)
    csv_info = write_convergence_csv(
        "Heat_2ph_2D_Robin_He$(params.He)_alpha$(params.alpha)_beta$(params.beta)",
...
@testset "Diphasic Heat 2D Robin convergence" begin
