...
u_left_case2(x) = x
u_right_case2(x) = 2.0 * x - 0.5

function build_solver_case2(mesh)
...
function run_liufedkiw_case2(nx_list; lx=1.0, x0=0.0)
    h_vals = Float64[]
    err_vals = Float64[]
    ...
    for nx in nx_list
        ...
        solver, capacity1, capacity2 = build_solver_case2(mesh)
        solve_DiffusionSteadyDiph!(solver; method=Base.:\)
        push!(solver.states, solver.x)

        _, _, global_errs, full_errs, cut_errs, empty_errs =
            check_convergence_diph(u_left_case2, u_right_case2, solver, capacity1, capacity2, 2, false)
...
function main(; csv_path=nothing, nx_list=nothing)
    ...
    data = run_liufedkiw_case2(nx_vals)
    csv_info = write_convergence_csv("LiuFedkiw_Case2", data; csv_path=csv_path)
    return (data = data, csv_path = csv_info.csv_path, table = csv_info.table)
end

results = main()

@testset "Liu-Fedkiw Diphasic Case 2" begin
