...
only (no plots). This variant sweeps Henry jumps and diffusivity ratios using a
dict of cases.
"""
...
function write_convergence_csv(method_name, data; csv_path=nothing)
    ...
    results_dir = isnothing(csv_path) ?
        joinpath(BENCH_ROOT, "results", "scalar", "diphasic", "extreme_regimes", "he_dict") :
...
@testset "Diphasic Heat 2D convergence (He + diffusivity cases)" begin
