...
function run_poisson2d(nx_list, ny_list; lx=2.0, ly=2.0, x0=-1.0, y0=-1.0)
    ...
    for (nx, ny) in zip(nx_list, ny_list)
        ...
        ndofs = (nx+1)*(ny+1)
        u0ₒ = [u_exact(mesh.nodes[1][i], mesh.nodes[2][j]) for j in 1:ny+1, i in 1:nx+1]
        u0ₒ = reshape(u0ₒ,:)
        u0ᵧ = zeros(ndofs)
        u0 = vcat(u0ₒ, u0ᵧ)
        ...
        Δx = lx / nx; Δy = ly / ny
        coverage_x = ceil(Int, 1.0 / Δx)
        coverage_y = ceil(Int, 1.0 / Δy)
...
@testset "Gibou-Fedkiw Poisson 2D" begin
