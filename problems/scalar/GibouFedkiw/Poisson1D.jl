...
φ_level(x,_=0) = abs(x) - 0.313
u_exact(x,_=0) = 4 * x^2 * sin(2π * x)
f_source(x,y,z) = begin
    term1 = 8 * sin(2π * x)
    term2 = 32π * x * cos(2π * x)
    term3 = -16π^2 * x^2 * sin(2π * x)
    term1 + term2 + term3
end

function run_poisson_1d(nx_list; lx=1.0, x0=-0.5)
    ...
    for nx in nx_list
        ...
        capacity = Capacity((x, _=0)->φ_level(x), mesh; method="ImplicitIntegration")
        operator = DiffusionOps(capacity)
        bc = Dirichlet((x,y,z)->u_exact(x))
        bc_b = BorderConditions(Dict(:left=>bc, :right=>bc))
        phase = Phase(capacity, operator, (x,y,z)->-f_source(x,y,z), (x,y,z)->1.0)

        ndofs = nx + 1
        u0ₒ = [u_exact(mesh.nodes[1][i]) for i in 1:ndofs]
        u0ᵧ = zeros(ndofs)
        u0 = vcat(u0ₒ, u0ᵧ)
...
function main(; csv_path=nothing, nx_list=nothing)
    ...
    data = run_poisson_1d(nx_vals)
    csv_info = write_convergence_csv("GibouFedkiw_Poisson1D", data; csv_path=csv_path)
    return (data=data, csv_path=csv_info.csv_path, table=csv_info.table)
end

results = main()

@testset "Gibou-Fedkiw Poisson 1D" begin
