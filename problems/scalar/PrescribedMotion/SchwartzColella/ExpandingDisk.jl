...
function expanding_disk(center)
...
function run_expanding_disk_convergence(
    nx_list::Vector{Int},
    ny_list::Vector{Int},
...
function main(; csv_path=nothing, nx_list=nothing, ny_list=nothing)
    nx_vals = isnothing(nx_list) ? [5, 8, 16, 32, 64, 128] : nx_list
    ny_vals = isnothing(ny_list) ? nx_vals : ny_list
    center = center_default()
    Tend = 0.1

    data = run_expanding_disk_convergence(nx_vals, ny_vals, center, Tend)
    csv_info = write_convergence_csv("SchwartzColella_ExpandingDisk", data; csv_path=csv_path)
    return (data = data, csv_path = csv_info.csv_path, table = csv_info.table)
end

results = main()

@testset "Schwartz-Colella expanding disk" begin
