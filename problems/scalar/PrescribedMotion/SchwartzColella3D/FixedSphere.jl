...
"""
3D Schwartz–Colella heat equation on a fixed sphere Ω₁ = {(x,y,z): r < 0.392}.
The exact solution is
    a(x,y,z,t) = 4 / (5π (t + 1)) * exp(-(x² + y² + z²)/(5 (t + 1))),
...
fixed_body(center) = (x,y,z,t)->sqrt((x-center[1])^2 + (y-center[2])^2 + (z-center[3])^2) - radius()
...
function run_fixed_sphere_convergence(
    nx_list::Vector{Int},
    ny_list::Vector{Int},
    nz_list::Vector{Int},
    ...
    for (nx, ny, nz) in zip(nx_list, ny_list, nz_list)
        ...
        Δt = 1.0 * min((lx / nx)^2, (ly / ny)^2, (lz / nz)^2)
        Tstart = Δt

        body = fixed_body(center)
...
function main(; csv_path=nothing, nx_list=nothing, ny_list=nothing, nz_list=nothing)
    ...
    data = run_fixed_sphere_convergence(nx_vals, ny_vals, nz_vals, center, Tend)
    csv_info = write_convergence_csv("SchwartzColella3D_FixedSphere", data; csv_path=csv_path)
    return (data=data, csv_path=csv_info.csv_path, table=csv_info.table)
end

results = main()

@testset "Schwartz-Colella 3D fixed sphere" begin
