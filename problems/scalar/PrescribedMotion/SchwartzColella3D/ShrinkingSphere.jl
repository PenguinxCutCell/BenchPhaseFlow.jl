...
"""
3D Schwartz–Colella heat equation on a shrinking sphere Ω₂(t) = {(x,y,z): r < 0.392 - t}.
The exact solution and source are identical to the fixed case, but the radius
decreases in time. Dirichlet data track the analytic solution along the moving
boundary and the outer box.
...
function shrinking_body(center)
    (x,y,z,t)->sqrt((x-center[1])^2 + (y-center[2])^2 + (z-center[3])^2) - (r_initial() - t)
end
...
function run_shrinking_sphere_convergence(
    nx_list::Vector{Int},
    ny_list::Vector{Int},
    nz_list::Vector{Int},
    ...
    for (nx, ny, nz) in zip(nx_list, ny_list, nz_list)
        ...
        body = shrinking_body(center)
        st_mesh = Penguin.SpaceTimeMesh(mesh, [0.0, Δt])
        capacity = Capacity(body, st_mesh; method="ImplicitIntegration")
        ...
        solve_MovingDiffusionUnsteadyMono!(solver, phase, body, Δt, Tstart, Tend, bc_b, interface_bc, mesh, "BE"; method=Base.:\, geometry_method="ImplicitIntegration")

        R_tend = max(r_initial() - Tend, 1e-3)
...
function main(; csv_path=nothing, nx_list=nothing, ny_list=nothing, nz_list=nothing)
    ...
    data = run_shrinking_sphere_convergence(nx_vals, ny_vals, nz_vals, center, Tend)
    csv_info = write_convergence_csv("SchwartzColella3D_ShrinkingSphere", data; csv_path=csv_path)
    return (data=data, csv_path=csv_info.csv_path, table=csv_info.table)
end

results = main()

@testset "Schwartz-Colella 3D shrinking sphere" begin
