...
const Lx, Ly = 2.0, 2.0
const x0, y0 = 0.0, 0.0
const channel_height = 0.6
const Umax = 1.0
const μ = 1.0
const ρ = 1.0

const angles_deg = [0, 5, 10, 15, 20, 25, 30, 35, 40, 45]
const angles_rad = [deg * π / 180 for deg in angles_deg]
...
function run_poiseuille_inclined()
...
function write_results_csv(results; csv_path=nothing)
    ...
    mkpath(out_dir)
    csv_file = isnothing(csv_path) ? joinpath(out_dir, "Poiseuille_2D_Inclined.csv") : csv_path
    CSV.write(csv_file, df)
    return csv_file
end

function main(; csv_path=nothing)
    results = run_poiseuille_inclined()
...
@testset "Poiseuille 2D inclined Navier–Stokes" begin
    @test !isempty(results.results)
    @test isfile(results.csv_path)
end

println("Benchmark completed. Results saved to ", results.csv_path)
