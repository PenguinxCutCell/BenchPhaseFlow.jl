...
"""
Unsteady Navier–Stokes benchmark: vortex shedding past a cylinder at Re = 100 (CSV only).

Mirrors the DFG 2D-3 setup (channel 2.2 × 0.41, cylinder radius 0.05 at (0.2, 0.2)).
Parabolic inflow with peak 1.5 gives mean velocity 1.0 → Re = 100 with ν = 1e-3.
Advances the unsteady Navier–Stokes mono solver, samples drag/lift and pressure drop,
computes statistics, and writes CSV diagnostics (no plots).
"""
...
const x0, y0 = 0.0, 0.0
const radius = 0.05
const diameter = 2radius
const cylinder_center = (0.2, 0.2)
circle_levelset = (x, y, _=0.0) -> radius - hypot(x - cylinder_center[1], y - cylinder_center[2])
...
# CSV output
###########
function write_results_csv(; csv_dir=nothing)
    ...
    stats_path = joinpath(out_dir, "CylinderRe100_Stats.csv")
    CSV.write(stats_path, stats_df)

    series_df = DataFrame(time = times[start_idx:end],
                          Cd = Cd_series[start_idx:end],
                          Cl = Cl_series[start_idx:end],
                          pdrop = pdrop_series[start_idx:end])
    series_path = joinpath(out_dir, "CylinderRe100_Series.csv")
    CSV.write(series_path, series_df)

    return (stats=stats_path, series=series_path)
end

function main(; csv_dir=nothing)
    paths = write_results_csv(; csv_dir=csv_dir)
    return (paths=paths, stats=Dict(
        :Cd_mean => Cd_mean,
        :Cd_max => Cd_max,
        :Cl_mean => Cl_mean,
        :Cl_rms => Cl_rms,
...
@testset "Cylinder Re100 unsteady" begin
    @test isfile(results.paths.stats)
    @test isfile(results.paths.series)
    @test isfinite(results.stats[:Strouhal])
end

println("Cylinder Re100 benchmark complete. Stats: ", results.paths.stats)
