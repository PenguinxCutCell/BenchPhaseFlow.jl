...
# Convergence study for 1D Stefan with least-squares interface update, sweeping max_iter.
...
function run_stefan_ls_convergence(nx_list::Vector{Int};
    lx::Float64=1.0,
    x0::Float64=0.0,
    Tstart::Float64=0.03,
    Tend::Float64=0.1,
    ...
    for nx in nx_list
        ...
        solver = StefanMono1D(phase, bc_b, bc_i, dt, u0, mesh, "BE")
        solver, residuals, xf_log, timestep_history = solve_StefanMono1D!(
            solver, phase, xf0, dt, Tstart, Tend, bc_b, bc_i, stef_cond, mesh, "BE";
            method=Base.:\,
...
function write_stefan_ls_csv(nx_list, data; csv_path=nothing)
    results_dir = isnothing(csv_path) ? joinpath(BENCH_ROOT, "results", "stefan") : dirname(csv_path)
    mkpath(results_dir)
    csv_out = isnothing(csv_path) ? joinpath(results_dir, "Stefan_1ph_ls_convergence.csv") : csv_path
...
function main(; nx_list=nothing, max_iter_list=nothing)
    ...
    iter_vals = isnothing(max_iter_list) ? [1, 2, 3, 4, 5, 6, 7] : max_iter_list

    for max_iter in iter_vals
        data = run_stefan_ls_convergence(nx_vals; max_iter=max_iter)
        csv_path = joinpath(BENCH_ROOT, "results", "stefan",
            "Stefan_1ph_ls_convergence_maxiter$(max_iter).csv")
        csv_info = write_stefan_ls_csv(nx_vals, data; csv_path=csv_path)
        println("Stefan 1ph LS convergence (max_iter=$max_iter) written to:\n  ", csv_info.csv_path)
    end
end
