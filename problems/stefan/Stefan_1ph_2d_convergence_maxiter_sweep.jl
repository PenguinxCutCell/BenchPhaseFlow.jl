...
# Convergence study for a 2D one-phase Stefan test case, sweeping max_iter.
...
function main(; nx_list=nothing, max_iter_list=nothing)
    ...
    for max_iter in iter_vals
        data = run_stefan_2d_convergence(nx_vals; max_iter=max_iter)
        csv_path = joinpath(BENCH_ROOT, "results", "stefan",
            "Stefan_1ph_2d_convergence_maxiter$(max_iter).csv")
        csv_info = write_stefan_2d_csv(nx_vals, data; csv_path=csv_path)
        println("Stefan 1ph 2d convergence (max_iter=$max_iter) written to:\n  ", csv_info.csv_path)
    end
end
